...
func New(conf *Config, metrics Metrics) *kafkaClient { //nolint:revive // New allows
	// returning unexported types as intended.
	err := validateConfigs(conf)
	if err != nil {
		slog.Error(fmt.Sprintf("could not initialize kafka, error: %v", err))

		return nil
	}

	if len(conf.Brokers) == 1 {
		slog.Info(fmt.Sprintf("connecting to Kafka broker: '%s'", conf.Brokers[0]))
	} else {
		slog.Info(fmt.Sprintf("connecting to Kafka brokers: %v", conf.Brokers))
	}
	...
	if err != nil {
		slog.Error(fmt.Sprintf("failed to connect to kafka at %v, error: %v", conf.Brokers, err))
...
func (k *kafkaClient) Publish(ctx context.Context, topic string, message []byte) error {
	ctx, span := otel.GetTracerProvider().Tracer("gocrux").Start(ctx, "kafka-publish")
	defer span.End()
	...
	if err != nil {
		slog.Error(fmt.Sprintf("failed to publish message to kafka broker, error: %v", err))
		return err
	}
...
func (k *kafkaClient) Subscribe(ctx context.Context, topic string) (*pubsub.Message, error) {
	...
	if k.config.ConsumerGroupID == "" {
		slog.Error("cannot subscribe as consumer_id is not provided in configs")

		return &pubsub.Message{}, ErrConsumerGroupNotProvided
	}

	ctx, span := otel.GetTracerProvider().Tracer("gocrux").Start(ctx, "kafka-subscribe")
	defer span.End()
	...
	if err != nil {
		slog.Error(fmt.Sprintf("failed to read message from kafka topic %s: %v", topic, err))
