...
func NewMetricsManager(meter metric.Meter) Manager {
...
func (m *metricsManager) NewUpDownCounter(name, desc string) {
	...
	if err != nil {
		slog.Error("Float64UpDownCounter", "err", err)
	...
	if err != nil {
		slog.Error("setUpDownCounter", "err", err)
	}
}
...
func (m *metricsManager) NewHistogram(name, desc string, buckets ...float64) {
	...
	if err != nil {
		slog.Error("WithExplicitBucketBoundaries", "err", err)
	...
	if err != nil {
		slog.Error("setHistogram", "err", err)
	}
}
...
func (m *metricsManager) NewGauge(name, desc string) {
	...
	if err != nil {
		slog.Error("Float64ObservableGauge", "err", err)
...
func (m *metricsManager) DeltaUpDownCounter(ctx context.Context, name string, value float64, labels ...string) {
	...
	if err != nil {
		slog.Error("getUpDownCounter", "err", err)
...
func (m *metricsManager) RecordHistogram(ctx context.Context, name string, value float64, labels ...string) {
	...
	if err != nil {
		slog.Error("getHistogram", "err", err)
...
func (m *metricsManager) getAttributes(name string, labels ...string) []attribute.KeyValue {
	...
	if labelsCount%2 != 0 {
		slog.Warn(fmt.Sprintf("metrics %v label has invalid key-value pairs", name))
	}

	cardinalityLimit := 20
	if labelsCount > cardinalityLimit {
		slog.Warn(fmt.Sprintf("metrics %v has high cardinality: %v", name, labelsCount))
	}
